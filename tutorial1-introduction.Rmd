...
Install the packages we'll be using today.

```{r}
# install.packages('reshape2','plyr','ggplot2','lme4','lmerTest','ez')
# commented out because I already have them
...
We can select one column and assign it to a particular vector variable.
...
We can also select rows by number.
...
We can also use summary functions to boil it down to a meaningful metric:
...
Or combine summary functions and mathematical operators to do useful things, like
calculate z-scores:
...
We'll be using simple ggplots in these tutorials, but the web documentation is great at helping you
...
We can pass specific options to these tests. One common option for 't.test' is var.equal=T so that we run a standard Student t-test.
